<x-admin-layout>
    ...
    <div class="d-flex justify-content-between  mb-3">
        <h1 class="display-4 border-bottom">Upraviť požívateľa</h1>
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="post" enctype="multipart/form-data">
        ...
        <!-- Name input -->
        <input class="form-control" type="text" name="name" placeholder="Meno" id="name" aria-label="default input example"  value="{{ old('name', $user->name) }}">
        <label class="form-label mb-3" for="name">Meno uživateľa</label>
        
        {{-- number of guest input --}}
        <input class="form-control" type="email" name="email" id="email" placeholder="Email" aria-label="default input example" value="{{ old('email', $user->email) }}">
